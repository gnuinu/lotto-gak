...
- 한 번의 추첨에서 게임끼리는 서로 다른 조합이어야 한다. `drawGames` 가 이미 나온
  조합을 거부하고 다시 뽑는다. 거부가 일어날 때만 난수를 더 쓰므로, **중복이
  없었던 시드의 결과는 그대로다**(시드 3000 × 조건 3종 = 9000건 중 달라진 건 2건이고
  둘 다 예전에 중복이 있던 경우였다). 그래서 스키마 버전은 올리지 않았다.
...
공은 원형, 숫자는 **흰색 볼드**. 어두운 배경(`--bg`) 위에서 대비를 잃지 않도록
`.ball` 클래스가 안쪽 그림자와 텍스트 그림자로 테두리를 만들어 준다. 특히
`#AAAAAA`(31–40) 위의 흰 숫자는 대비가 빠듯하므로 `text-shadow` 를 지우지 말 것.

## 디자인 토큰과 색 규칙

테마는 "심야 복권판매점"이되 **차분한 쪽**이다. 초기 버전의 네온 사인 스타일
(형광 핑크 + 글로우 + 강조색 3개)은 촌스럽고 눈이 아파서 걷어냈다.
기준은 취향이 아니라 측정이다.

**하드 규칙**

1. **강조색은 하나(`--accent` 보라)만 쓴다.** 로또 공이 이미 규격 5색을 쓰므로 화면의
   나머지는 물러서야 한다. 금색(`--gold`)은 1등 표시에만, 빨강(`--danger`)은 실패·삭제에만.
   포인트 컬러를 하나 더 만들고 싶으면 먼저 기존 것을 지워라.
2. **모든 글자색은 배경 대비 4.5:1 이상.** 특히 하단 고지 문구(`--text-3`, 6.3:1)는
   법적 고지이므로 절대 더 흐리게 만들지 않는다. 예전 값은 3.71:1 로 미달이었다.
   색을 바꾸면 대비를 **계산해서** 확인한다(눈으로 판단하지 말 것).
3. **네온 글로우 금지.** 번지는 `text-shadow` / `box-shadow` 로 강조하지 않는다.
   깊이는 표면 밝기로 만든다: `--bg` → `--surface` → `--surface-2` → `--surface-3`
   순으로 밝아진다(다크 테마에서 그림자는 잘 안 보인다).
4. **채도를 낮춘다.** 어두운 배경 위의 고채도 색은 번져 보인다.
   강조색 81%, 금색 75%로 낮춰 뒀다(예전엔 둘 다 100%).
5. **임의값 대신 토큰만.** 글자 크기는 `--fs-xs`~`--fs-2xl`, 모서리는
   `--r-sm`/`--r-md`/`--r-lg`/`--r-full`. `11.5px`, `13.5px` 같은 값을 새로 만들지 않는다.
6. **숫자에는 `font-variant-numeric: tabular-nums`.** 회차·시드·합계가 흔들리지 않게.
7. **초점 표시(`:focus-visible`)를 지우지 않는다.** 키보드 사용자에게 유일한 단서다.
8. 작은 보조 버튼은 `.btn-mini` 를 쓴다. `.history-item__btn` 은 같은 것의 옛 이름이며
   호환을 위해 같은 규칙에 묶여 있다 — 새 코드에서는 `.btn-mini` 를 쓸 것.

## UX 규칙

- **하단 바 높이를 상수로 박지 않는다.** 고지 문구는 화면 폭·글자 크기에 따라 줄 수가
  달라진다. `BottomBar` 가 `ResizeObserver` 로 실제 높이를 재서 `--bottom-bar-h` 에
  넣고, 본문은 그만큼 아래 여백을 둔다. 상수로 두었더니 실제로 콘텐츠가 가려졌다.
- **추첨하면 결과 화면으로 데려간다.** 조건 탭에서 추첨 버튼을 눌렀는데 그 자리에
  머무르면 아무 일도 안 일어난 것처럼 보인다. 성공이든 실패든 `tab: 'draw'` 로 옮긴다.
- **되돌릴 수 없는 동작은 한 번 더 묻는다.** 이력 전체 삭제는 2단계 확인을 거친다.
  반대로 항목마다 반복되는 삭제 버튼이 화면에서 제일 튀면 안 된다 — 테두리 없이 조용히.
- **누를 수 있는 것은 최소 34px 높이.**
- **통계는 막대로, 색은 하나로.** 45개 번호를 색으로 구분하려 들면 아무것도 안 읽힌다.
  정체성은 공(규격 색)이, 크기는 단색 막대가 맡는다. 막대는 **0부터** 시작한다 —
  잘라내면 번호 간 차이가 실제보다 커 보이고, 그건 "이 번호가 유리하다"는 거짓말이 된다.
...
- 이 상황에 대응하는 장치: 브라우저에 가까운 헤더, 본문을 text 로 먼저 받아 HTML 을
  판정하는 `looksLikeHtml()`, `--probe` 진단 모드, 시크릿 `LOTTO_API_URL` 로 조회 주소 교체.
  응답을 `response.json()` 으로 바로 읽지 말 것 — 파싱 에러만 남고 무엇이 왔는지 사라진다.
...
- 내 번호 대조는 "저장된 번호를 그 회차 결과와 견주어 본 것" 이다.
  실제 구매를 뜻하지 않으므로 문구도 그렇게 유지한다. 직접 입력 대조
  (`ManualCheck`)도 마찬가지다 — 입력한 번호는 `store.manualNumbers` 에만 있고
  localStorage 에 남기지 않는다(저장 키는 이력 하나뿐이라는 규칙을 지킨다).
